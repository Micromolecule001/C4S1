                    ...
                    int current_arg_idx)
{
  ...
  if (add_edit && (!did_next || wp->w_arg_idx_invalid)) {
    ...
    if (bt_help(wp->w_buffer)) {
      ...
      // Recreate a terminal buffer. We cannot restore the live job or scrollback,
      // but we can reopen a fresh terminal in this window. The working directory
      // for the window/tab is restored elsewhere in this function.
      if (fprintf(fd,
                  "terminal\n"
                  // Fixup :terminal buffer name to preserve the original title. #7836
                  "if &buftype ==# 'terminal'\n"
                  "  silent file %s\n"
                  "endif\n",
                  fname_esc) < 0) {
